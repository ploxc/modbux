// `types/server.ts` imports `getAddressFitError`, `getValueRangeError` and
// `MAX_UTF8_LENGTH` from here, so this reads the barrel no longer: it would be a
// cycle through `types/index.ts`. `import type` says which names the compiler
// erases, and the cycle back through `types/server.ts` is one of them.
// `DataTypeSchema` is the one value below, out of a module that imports zod and
// nothing else.
import { DataTypeSchema, type DataType, type EncodableDataType } from './types/datatype'
import type { RegisterParams } from './types/server'
