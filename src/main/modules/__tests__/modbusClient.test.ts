...
describe('ModbusClient', () => {
  ...
  describe('lastRead', () => {
    // The window gets the rows through IPC, where a Buffer arrives as JSON, so
    // the two are compared on what a row says rather than on the object.
    const rowsOf = (rows: { id: number; hex: string }[] | undefined) =>
      rows?.map(({ id, hex }) => ({ id, hex }))

    it('holds what the last read sent, with its type, unit id and time', async () => {
      await connectClient()
      appState.updateConnectionConfig({ unitId: 7 })
      setupHoldingRegisterReadMock([100, 200])
      ...
      const sent = getWindowCalls('register_data')[0]?.[1]
      expect(client.lastRead).toMatchObject({
        type: 'holding_registers',
        unitId: 7,
        time: Date.parse('2026-09-25T12:00:00Z')
      })
      expect(rowsOf(client.lastRead?.registerData)).toEqual(rowsOf(sent))
      expect(rowsOf(sent)).toHaveLength(2)
    })

    it('is replaced by the next read', async () => {
      await connectClient()
      setupHoldingRegisterReadMock([100])
      await client.read()
      setupHoldingRegisterReadMock([300])
      await client.read()

      const sent = getWindowCalls('register_data')[1]?.[1]
      expect(rowsOf(client.lastRead?.registerData)).toEqual(rowsOf(sent))
      expect(rowsOf(sent)?.[0]?.hex).toBe('012c')
    })

    it('holds nothing before the first read', () => {
      expect(client.lastRead).toBeUndefined()
    })

    it('is not what a register scan found', async () => {
      await connectClient()
      setupHoldingRegisterReadMock([5])

      await client.scanRegisters({ addressRange: [0, 9], length: 10, timeout: 1000 })

      expect(getWindowCalls('register_data').length).toBeGreaterThan(0)
      expect(client.lastRead).toBeUndefined()
    })

    it('is not a read a stopped poll threw away', async () => {
      await connectClient()
      const gates = gateTheReads()
